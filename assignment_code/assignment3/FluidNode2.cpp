...
namespace GLOO {
	...
	FluidNode2::FluidNode2(IntegratorType type, float step_size) {
	...
	int FluidNode2::IndexOf(int i, int j, int z) {
		return dimension_ * dimension_ * i + dimension_ * j + z;
	}

	void FluidNode2::Setup() {
	...
	void FluidNode2::drawBox(bool filled) {
	...
	void FluidNode2::Update(double delta_time) {
		...
		for (float i = 0; i <= int(delta_time / step_size_); i++) {
			...
			for (int i = 0; i < state_.positions.size(); i++) {
				if (state_.positions[i][1] < 0.00 && state_.positions[i][0] >= 0.00
					&& state_.positions[i][0] < box_dim_ && state_.positions[i][2] >= 0.00
